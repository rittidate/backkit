...
class Kt_Order extends Pager
{
    ...
    private function _print($where="", $datatype=null, $data=null){
        ...
        $SQL = " SELECT tb.*, ship.data_type_name as shipment, pay.data_type_name as payment,ord.data_type_name as order_status
	                FROM [pf]{$this->tb} AS tb
        ...
        while($row = $result->fetchRow()) {
            ...
            $response->rows[$i]['cell']=array($row->id, $orderDate->format('d/m/y H:i'), $row->firstname, $row->shipment, $row->payment, $row->shipprice, $row->subtotal, $row->state, $row->mobile, $row->order_status, $action);
            $i++;
        }
        return $response;
    }
    ...
    public function add()
    {
        ...
        $this->checkEmailOrderStatus($order_status, $id, $email, $postcode, $aDetail, $json);
    ...
    public function checkEmailOrderStatus($order_status, $id, $email, $postcode = '', $aDetail = array(), $json = array()){
        switch ($order_status)
        {
            case 10:
                $status = "Order Start";
